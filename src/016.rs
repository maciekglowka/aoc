...
fn main () {
    ...
    let start = State {
        positions: ["AA".to_string(), "AA".to_string()],
    ...
    for step in 0..26 {
        ...
        for state in current_gen.iter() {
            ...
            for combination in combinations.into_iter() {
                ...
                if combination.0.1 { open.insert(combination.0.0.clone()); }
                if combination.1.1 { open.insert(combination.1.0.clone()); }
        ...
        current_gen = next_gen.into_iter().rev().take(100000).collect();
        println!("Step {}", step + 1);
    }
...
//     positions: &[String; 2]
// ) -> HashSet<String> {
//     let mut actions: HashSet<String> = positions.iter()
//         .map(|p| valves[p].connections)
//         .flatten()
//         .collect();
//     // if !open_valves.contains(current_valve) {
//     //     actions.push(current_valve.clone());
//     // }
//     actions.insert(positions[0]);
//     actions.insert(positions[1]);
//     actions
// }
